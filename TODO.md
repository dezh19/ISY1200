...
- [x] Update `netlify.toml` NODE_VERSION from `18` to `20.9.0`.
- [x] Add `"engines": { "node": ">=20.9.0" }` to `package.json`.
- [ ] Commit and push to trigger a fresh Netlify deploy.

## UI visibility fix
- [x] Fix "The Key Takeaway" text visibility in `components/AnalogySection.tsx`.
- [x] Fix stats text visibility in `components/WhyUGSection.tsx` ("Years of Excellence", "Computing Programmes", "ACM", "CARICOM").
